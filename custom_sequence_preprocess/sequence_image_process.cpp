...
// Derive the temporal-sequence key for a preprocess unit.
//
// In object mode the plugin fills `unit.roi_meta.object_meta` with the source
// object, whose `object_id` is the tracker's stable ID. Keying on that (rather
// than the bounding-box position) keeps a moving person's sequence intact across
// frames, so that even a person whose bbox changes every frame can accumulate the
// `_seqSize` frames needed to emit one action inference. In frame/ROI mode the
// object meta is NULL, so we keep the original positional key.
SourceKey
getUnitKey(const NvDsPreProcessUnit& unit)
{
    uint64_t source_id = unit.frame_meta->source_id;
    const NvDsRoiMeta& roiMeta = unit.roi_meta;
    if (roiMeta.object_meta) {
        return std::make_tuple(
            source_id, (int64_t)roiMeta.object_meta->unique_component_id,
            (int64_t)roiMeta.object_meta->object_id);
    }
    const auto& roi = roiMeta.roi;
    return getKey(source_id, roi.left, roi.top, roi.width, roi.height);
}
...
// Reclaim pooled-buffer slots of source sequences that have gone inactive (no
// new frames written) for `staleFrames` consecutive frames, and release the
// pooled buffer of any block left with no active sources. Reclaimed slots are
// marked free for reuse by new sources. In per-object mode each distinct tracked
// object occupies one slot; without per-slot reclamation these accumulate for
// every object seen over the whole stream and exhaust the pool, causing
// acquire() to block forever. This bounds pooled-buffer usage by the maximum
// number of concurrently active objects instead of total objects ever seen.
void
BufferManager::reclaimInactiveSources(uint64_t currentFrameNo, uint32_t staleFrames)
{
    if (_src2dstMap.empty()) {
        return;
    }
    std::vector<SourceKey> staleKeys;
    for (const auto& pair : _src2dstMap) {
        const RoiProcessedBuf* roiBuf = pair.second.get();
        bool isLive = (roiBuf->latestFrameNo == FRAME_NO_NOT_SET) ||
                      (currentFrameNo < roiBuf->latestFrameNo + staleFrames);
        if (!isLive) {
            staleKeys.push_back(pair.first);
        }
    }
    for (const auto& key : staleKeys) {
        auto it = _src2dstMap.find(key);
        if (it == _src2dstMap.end()) {
            continue;
        }
        RoiProcessedBuf* roiBuf = it->second.get();
        FullBatchBlock* block = roiBuf->block;
        uint32_t slot = (uint32_t)(roiBuf->blockOffset / perBatchBytes());
        block->freeSlots.push_back(slot);
        _src2dstMap.erase(it);
        LOG_INFO(
            "Reclaimed inactive source slot, freed slot: %u, total sources: %u",
            (uint32_t)slot, (uint32_t)_src2dstMap.size());
    }
    if (_accumulateBlocks.empty()) {
        return;
    }
    for (auto it = _accumulateBlocks.begin(); it != _accumulateBlocks.end();) {
        FullBatchBlock* block = it->get();
        bool hasMembers = false;
        for (const auto& pair : _src2dstMap) {
            if (pair.second->block == block) {
                hasMembers = true;
                break;
            }
        }
        if (!hasMembers) {
            if (block->buf && _allocator) {
                _allocator->release(block->buf);
                block->buf = nullptr;
            }
            it = _accumulateBlocks.erase(it);
            LOG_INFO("Reclaimed empty sequence block, released pooled buffer");
        } else {
            ++it;
        }
    }
...
// store at most `max-batch-size` ROIs. If a stale slot was reclaimed, it is
// reused first so the number of pooled buffers stays bounded by the number of
// concurrently active objects rather than the total objects ever seen.
...
BufferManager::addRoi(const SourceKey& key, const NvDsPreProcessUnit& unit)
{
    ...
    uint32_t slot = 0;
    // Prefer reusing a reclaimed (free) slot from any existing block.
    for (auto& blk : _accumulateBlocks) {
        FullBatchBlock* b = blk.get();
        if (!b->freeSlots.empty()) {
            block = b;
            slot = b->freeSlots.back();
    ...
    if (!block) {
        ...
        block = (*_accumulateBlocks.rbegin()).get();
        slot = block->inUseBatchSize;
    ...
    newSrc->blockOffset = perBatchBytes() * slot;
    ...
    UNUSED(srcId);
    LOG_DEBUG(
        "Adding new ROI of source: %u mapped to dst block slot: %u, "
        "total ROI numbers: %d",
        (uint32_t)srcId, slot, (uint32_t)_src2dstMap.size());
    ...
    CustomTensorParams& tensorParam, NvDsPreProcessAcquirer* allocator)
{
    ...
    // reclaim pooled buffers of source sequences that have gone inactive, so
    // the tensor pool stays bounded by concurrent active objects.
    if (!batchIn->units.empty()) {
        uint64_t currentFrameNo = batchIn->units.back().frame_num;
        _bufManager->reclaimInactiveSources(currentFrameNo, _S * 2);
    }
