...
class VehicleMonitorFilter:
    ...
    def defineROI(self, img_height, img_width):
        roi_points = [  Point( (0.4*img_width), 0 ), Point( (0.6*img_width), 0 ), Point( (0.6*img_width), img_height ), Point( (0.4*img_width), img_height ) ]
        ...
        for object in frame_metadata.get_object_list():
            ...
            if self.roi.checkInside(center_point):
                ...
                if self.vehicleHistoricalRecord.triggerAlert(track_id=object_id, centerPoint=center_point):
                    ...
                    if (horizontal_motion > 100): # filter out cases where the horizotal motion is less than 100 pixels
                        metadata = {}
                        ...
                        #metadata['snapshot'] = self.take_snapshot(gst_buffer = gst_buffer, frame_meta = frame_meta, points = entry_exit_points)
                        print(metadata)
                        if object.get_object_label() == 'Car':
                            if object.get_object_classification() != []:
                                print(object.get_object_classification())
                        #self.save_snapshot(file_name = "ROIentryexit_frame_" + str(frame_metadata.frame_number) + "_" + str(object_id) + ".jpg", frame = frame_metadata.get_frame(), points = entry_exit_points)
