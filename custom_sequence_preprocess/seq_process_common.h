...
struct FullBatchBlock {
    ...
    // slot indices (within this block) that have been reclaimed and are free
    // for reuse by new sources. Keeps pooled buffer usage bounded by the number
    // of concurrently active objects instead of total objects ever seen.
