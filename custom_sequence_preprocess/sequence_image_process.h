...
public:
    ...
    // clear before quit
    void clearAll();
    // release pooled buffer blocks whose member source sequences have all been
    // inactive for `staleFrames` consecutive frames. Keeps the tensor buffer
    // pool bounded by the number of concurrently active tracked objects.
    void reclaimInactiveSources(uint64_t currentFrameNo, uint32_t staleFrames);
